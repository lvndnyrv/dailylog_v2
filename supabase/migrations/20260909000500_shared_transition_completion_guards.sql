-- Keep legacy/mobile callers on the same checks as the reviewed web action.
alter function public.complete_room_transition_plan(uuid) rename to _complete_room_transition_effects;
revoke all on function public._complete_room_transition_effects(uuid) from public,anon,authenticated,service_role;
...
begin
  ...
  if plan.status='completed' then return public._complete_room_transition_effects(plan.id); end if;
  ...
  return public._complete_room_transition_effects(plan.id);
end; $$;
...
create function public.complete_room_transition_plan(p_plan_id uuid)
returns jsonb language plpgsql security definer set search_path=public as $$
declare version timestamptz;
begin
  if auth.uid() is null or not public.is_admin() or not public.has_permission('children','edit') then raise exception 'Administrator children-edit permission required'; end if;
  perform pg_advisory_xact_lock(hashtextextended(public.get_my_daycare_id()::text,0));
  select updated_at into version from public.room_transition_plans
    where id=p_plan_id and daycare_id=public.get_my_daycare_id() for update;
  if not found then raise exception 'Plan not found'; end if;
  return public.complete_reviewed_room_transition(p_plan_id,version);
end; $$;
revoke all on function public.complete_room_transition_plan(uuid) from public,anon;
grant execute on function public.complete_room_transition_plan(uuid) to authenticated;
