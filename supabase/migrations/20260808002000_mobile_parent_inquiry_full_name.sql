-- Parent mobile group 26 follow-up: preserve the child's complete name and
-- read tour-host employment titles from staff_members (profiles intentionally
-- contains identity/contact data only).

drop function if exists public.submit_parent_enrollment_inquiry(
  uuid, text, text, text, text, date, uuid, date, int
);

create function public.submit_parent_enrollment_inquiry(
  p_daycare_id uuid,
...
as $$
declare
  v_full_name text := regexp_replace(btrim(coalesce(p_child_full_name, '')), '\s+', ' ', 'g');
  v_first_name text;
  v_last_name text;
...
begin
  if v_full_name = '' then
    raise exception 'Child full name is required';
  end if;
  if length(v_full_name) > 160 then
    raise exception 'Child full name is too long';
  end if;
  ...
  set child_last_name = v_last_name
  where enrollment.id = v_enrollment_id
  returning enrollment.offer_code into v_code;
  ...
  select jsonb_build_object(
    ...
    'tour', (
      ...
      left join public.staff_members staff
        on staff.profile_id = host.id and staff.daycare_id = slot.daycare_id
    ...
    'open_tour_slots', coalesce((
      select jsonb_agg(jsonb_build_object(
        'id', available.id,
        'starts_at', available.starts_at,
        'ends_at', available.ends_at,
        'host_name', available.host_name,
        'host_title', available.host_title
      ) order by available.starts_at)
      from (
        select slot.id, slot.starts_at, slot.ends_at,
          host.full_name as host_name, staff.job_title as host_title
        from public.enrollment_tour_slots slot
        left join public.profiles host on host.id = slot.host_id
        left join public.staff_members staff
          on staff.profile_id = host.id and staff.daycare_id = slot.daycare_id
...
-- A rescheduled slot becomes available again with its original host. The
-- first Group 26 migration cleared host_id, which made the reopened time look
-- unstaffed to the next family.
