-- The Group 9 dashboard migration added new notification kinds by replacing
-- this shared guard with a shorter version. Restore the narrow parent-action
-- checks and enrollment routing while retaining every Group 9 permission map.
...
begin
  ...
  v_area := case
    ...
    when new.kind in (
      'invoice', 'invoice_reminder', 'payment',
      'parent_payment_receipt', 'parent_enrollment_receipt'
    ) then 'billing'
