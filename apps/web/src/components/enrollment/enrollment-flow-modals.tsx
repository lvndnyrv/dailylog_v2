...
export function EnrollmentLifecycleModal({ enrollment, onClose }: { enrollment: Enrollment; onClose: () => void }) {
  const steps = jsonObject(enrollment.onboarding_steps);
  const complete = [Boolean(enrollment.offer_sent_at), enrollment.offer_status === "accepted" || enrollment.stage === "enrolled", documentRows(enrollment.documents_status).every((item) => item.status === "received"), Boolean(steps.room_schedule), Boolean(steps.first_day)];
  return (
    <Modal onClose={onClose} width={600}>
      ...
      <div className="flex flex-col">
        {[
          ["Offer sent", enrollment.offer_sent_at ? `Sent ${formatDateTime(enrollment.offer_sent_at)} · spot held immediately` : "Not sent yet"],
          ["Offer accepted & deposit", enrollment.offer_status === "accepted" || enrollment.stage === "enrolled" ? `${money(enrollment.offer_deposit_cents ?? 0)} recorded and credited` : "Waiting for family acceptance"],
          ["Paperwork complete", `${documentRows(enrollment.documents_status).filter((item) => item.status === "received").length} of 4 documents received`],
          ["Room & schedule", String(steps.room_schedule ?? "Assign the room, schedule and primary educator")],
          ["First day", enrollment.desired_start_date ? `${formatDate(enrollment.desired_start_date)} · roster, welcome and billing start together` : "Choose a first day"],
        ].map(([title, detail], index) => (
          <div key={title} className="flex gap-3 border-b border-[#EDF3FB] py-3 last:border-0"><span className={`grid size-7 shrink-0 place-items-center rounded-full text-[11px] font-bold ${complete[index] ? "bg-[#E4F3EC] text-success" : "bg-tint text-primary"}`}>{complete[index] ? "✓" : index + 1}</span><span><b className="block text-[12.5px] text-ink">{title}</b><span className="text-[11.5px] text-muted">{detail}</span></span></div>
        ))}
      </div>
      <Notice tone="success">Nothing is double-booked: the room count updates when the offer is accepted and enrollment is completed.</Notice>
  ...
  return (
    <Modal onClose={onClose} width={430}>
      ...
      {state.ok ? <><Notice tone="success">Document request queued for delivery.</Notice><Button type="button" onClick={onClose}>Done</Button></> : (
        <form action={action} className="flex flex-col gap-4">
          <input type="hidden" name="enrollment_id" value={enrollment.id} />
          <fieldset className="flex flex-col gap-2"><legend className="mb-1 text-[13px] font-bold text-ink">Still missing</legend>{missing.map((document, index) => <label key={document.key} className={`${card} flex items-center gap-3 px-3 py-2.5`}><input type="checkbox" name="documents" value={document.key} defaultChecked={index === 0} className="size-4 accent-primary" /><span className="flex-1"><b className="block text-[12px] text-ink">{document.label}</b><span className="text-[10.5px] capitalize text-faint">{document.status}</span></span></label>)}</fieldset>
          <label className="flex flex-col gap-1.5"><span className="text-[13px] font-bold text-ink">Message</span><textarea name="message" rows={3} defaultValue={`Hi ${enrollment.guardian_name?.split(" ")[0] ?? "there"} — one last thing before ${enrollment.child_first_name ?? "your child's"} start date. Please upload the missing form from your application. Thank you!`} className="rounded-[13px] border-[1.5px] border-[#D6E1F0] p-3 text-[12.5px] text-ink outline-none focus:border-primary" /></label>
          ...
          {state.error && <Notice tone="error">{state.error}</Notice>}
          <ModalButtons onClose={onClose} pending={pending} submit="Send request" />
        </form>
      )}
    </Modal>
  );
  ...
  return (
    <Modal onClose={onClose} width={430}>
      ...
      {state.ok ? <><Notice tone="success"><b>Offer sent.</b> The spot is now held until the deadline.</Notice><Button type="button" onClick={onClose}>Done</Button></> : (
        <form action={action} className="flex flex-col gap-4">
          <input type="hidden" name="enrollment_id" value={enrollment.id} />
          <Select label="The room" name="classroom_id" defaultValue={room?.id ?? ""} options={rooms.map((item) => ({ id: item.id, name: `${item.name} · ${Math.max(0, Number(item.capacity ?? 0) - Number(item.enrolled_count))} spots available` }))} empty="Choose room" required />
          ...
          <Notice tone="success">The deposit is credited to the first month. If this offer closes, the configured auto-offer rule can advance the next family.</Notice>
          {state.error && <Notice tone="error">{state.error}</Notice>}
...
function documentRows(value: unknown) {
  ...
  return [
    { key: "immunization", label: "Immunization record", status: String(data.immunization ?? "missing") },
    { key: "emergency_contacts", label: "Emergency contacts", status: String(data.emergency_contacts ?? "missing") },
    { key: "medical", label: "Allergy & medical form", status: String(data.medical ?? "missing") },
    { key: "handbook", label: "Signed parent handbook", status: String(data.handbook ?? "missing") },
  ];
}
